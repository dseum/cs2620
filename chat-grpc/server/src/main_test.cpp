...
public:
    ...
    void resetDatabase() {
        ...
        execute("CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, password_encoded TEXT, is_deleted INTEGER DEFAULT 0)");
        execute("CREATE TABLE conversations (id INTEGER PRIMARY KEY AUTOINCREMENT, is_deleted INTEGER DEFAULT 0)");
        execute("CREATE TABLE conversations_users (conversation_id INTEGER, user_id INTEGER)");
        execute("CREATE TABLE messages (id INTEGER PRIMARY KEY AUTOINCREMENT, conversation_id INTEGER, user_id INTEGER, content TEXT, is_read INTEGER, is_deleted INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
    }
};
...
BOOST_AUTO_TEST_CASE(test_signin_user) {
    ...
    grpc::Status signup_status = stub->SignupUser(&context1, signup_request, &signup_response);
    ...
    grpc::Status signin_status = stub->SigninUser(&context2, signin_request, &signin_response);
...
BOOST_AUTO_TEST_CASE(test_send_message) {
    ...
    grpc::Status signup_status1 = stub->SignupUser(&context1, signup_request1, &signup_response1);
    ...
    grpc::Status signup_status2 = stub->SignupUser(&context2, signup_request2, &signup_response2);
    ...
    grpc::Status create_conv_status = stub->CreateConversation(&context3, create_conv_request, &create_conv_response);
    ...
    send_msg_request.set_conversation_id(create_conv_response.conversation_id());
    send_msg_request.set_send_user_id(signup_response1.user_id());
    send_msg_request.set_content("Hello, user2!");

    grpc::Status send_msg_status = stub->SendMessage(&context4, send_msg_request, &send_msg_response);
...
BOOST_AUTO_TEST_CASE(test_delete_user) {
    ...
    grpc::Status signup_status = stub->SignupUser(&ctx_signup, signup_req, &signup_res);
    BOOST_REQUIRE(signup_status.ok());
    ...
    grpc::Status del_status_wrong = stub->DeleteUser(&ctx_wrong, del_req_wrong, &del_res_wrong);
    ...
    grpc::Status signin_status = stub->SigninUser(&ctx_signin, signin_req, &signin_res);
    BOOST_CHECK(!signin_status.ok());
}
...
BOOST_AUTO_TEST_CASE(test_get_other_users) {
    ...
    auto createUser = [&](const std::string& username, const std::string& password) -> int64_t {
    ...
    BOOST_CHECK(std::find(returned_ids.begin(), returned_ids.end(), idB) != returned_ids.end());
    BOOST_CHECK(std::find(returned_ids.begin(), returned_ids.end(), idC) != returned_ids.end());
...
BOOST_AUTO_TEST_CASE(test_create_conversation) {
    ...
    // Test error case: attempt to create a conversation with a non-existent other user.
    ...
    grpc::Status status_err = stub->CreateConversation(&ctx4, conv_req_err, &conv_res_err);
...
BOOST_AUTO_TEST_CASE(test_get_conversations) {
    ...
    BOOST_REQUIRE(stub->CreateConversation(&ctx3, create_conv_req, &create_conv_res).ok());
    ...
    BOOST_REQUIRE(stub->GetConversations(&ctx5, get_conv_req, &get_conv_res).ok());
    BOOST_CHECK_EQUAL(get_conv_res.conversations_size(), 1);
    auto conv = get_conv_res.conversations(0);
    BOOST_CHECK_EQUAL(conv.conversation_id(), create_conv_res.conversation_id());
    ...
    BOOST_REQUIRE(stub->GetConversations(&ctx6, get_conv_req2, &get_conv_res2).ok());
...
BOOST_AUTO_TEST_CASE(test_delete_conversation) {
    ...
    BOOST_REQUIRE(stub->DeleteConversation(&ctx4, del_conv_req, &del_conv_res).ok());

    // Get conversations for user1; the deleted conversation should not be returned.
    grpc::ClientContext ctx5;
    GetConversationsRequest get_conv_req;
    GetConversationsResponse get_conv_res;
...
BOOST_AUTO_TEST_CASE(test_receive_message) {
    ...
    std::unique_ptr<grpc::ClientReader<converse::MessageWithConversationId>> reader(
        stub->ReceiveMessage(&stream_ctx, stream_req));
    ...
    // Wait for the message to be received (or timeout).
    {
        ...
        cv.wait_for(lock, std::chrono::seconds(2), [&]() { return got_message; });
    }
