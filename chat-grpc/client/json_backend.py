...
from protocols.json.deserialize import deserialize
from protocols.json.client import (
    SignupUserRequest,
    SigninUserRequest,
    SignoutUserRequest,
    DeleteUserRequest,
            ...
            while True:
                ...
                if self.expected_size is None:
                    if len(self.buffer) >= header_size:
                        ...
                        self.expected_size, _ = SizeWrapper.deserialize_from(
                            self.buffer, 0
                        )
        ...
        match body.operation_code:
            ...
            case OperationCode.CLIENT_GET_OTHER_USERS:
                if body.status_code == StatusCode.FAILURE:
                    ...
                    return
                # users = [User(user[0], user[1]) for user in body.users]     #This line is going to be commented out for json
                users = [User(user["id"], user["username"]) for user in body.users]
